...
public class StudentView {
	...
	private void allStudent() {
		...
		for(StudentDTO std : students) {
			 ...
			 * 위에 코드가 System.out.println(std.toString()); 
			 * 와 같이 실행 되는 이유?
			 * 
			 * 참조 변수를
			 * 문자열 만들 때
	 ...
	 * 	-> 추가 성공 시 ture, 실패 시 false 반환 받아 결과 출력
	 * </pre>
	 */
	...
	private void selectName() {
		...
		StudentDTO student = service.selectName(inputName);
		
		if(student == null) {
			System.out.println(inputName + " 학생은 존재하지 않습니다");
			return;
		}
	 ...
	 * 
	 * 	- 인덱스가 정상 입력 되었다면
	 * 	해단 인덱스 번째 학생(StudentDTO)을 반환 받기
	 * 
	 * 	- 합계, 평균 구하기
	 * 
	 * 	- 아래와 같은 형식으로 출력
